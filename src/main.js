...
Vue.prototype.$getFromBucket = async (fileName) => {
    try {
        const response = await Vue.prototype.$http.get(Vue.prototype.$bucketUrl + fileName)
        return response
    } catch (ex) {
        console.log(ex);
        return null
...
function commonmarkRenderPlaintextRecursive(root) {
    ...
    let out = ['paragraph', 'block_quote', 'code_block', 'heading', 'softbreak', 'linebreak'].includes(root.type) ? ' ' : ''
...
Vue.prototype.$formatDate = (date) => date.calendar(null, {
    sameDay: '[Today] h:mmA',
